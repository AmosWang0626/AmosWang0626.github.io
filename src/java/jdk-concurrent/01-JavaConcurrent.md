...
2. 解决问题
   ...
   可见性：锁释放之前，对共享变量的修改，会刷回主存，对于随后获取该锁的线程是可见的。
...
- 可重入性
  ...
  同一线程可重复获取同一把锁，通过锁计数器实现（每次加锁+1，释放-1，计数器为0时完全释放）。

  可重入的计数器在哪存储呢？这取决于锁的状态（偏向锁、轻量级锁、重量级锁）

  - 偏向锁：无显式计数器，通过对象头的偏向线程ID隐式判断。
  - 轻量级锁：通过栈帧中的锁记录数量隐式记录重入次数。
  - 重量级锁：通过 Monitor 对象的 _recursions 字段显式维护计数器。
